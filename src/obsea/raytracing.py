...
def munk(z, c0=1492.0, zc=1300.0, B=1300.0, gamma=1.14e-5):
    epsilon = B * gamma / 2
    zbar = 2 * (z - zc) / B
    c = c0 * (1.0 + epsilon * (zbar - 1 + np.exp(-zbar)))
    dcdz = c0 * epsilon * (2 / zc) * (1 - np.exp(-zbar))
    return [c, dcdz]


def equivalent_celerity(ssp, depth):
    t = quad(lambda z: 1 / ssp(z)[0], 0, depth)[0]
    return depth / t
...
def trace_direct(ssp, depth, n, smax, **kwargs):
    ...
    for k in range(n):
        ...
        sol = solve_ivp(fun, (0, smax), y0, events=[hit_ground], **kwargs)
